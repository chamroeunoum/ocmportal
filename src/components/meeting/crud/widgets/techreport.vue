      ...
      <div class="w-11/12 min-h-screen relative border border-gray-300 bg-gray-50" >
        <div class="w-96 min-h-screen absolute left-0 top-0 bottom-0" >
          ...
          <div class="absolute left-0 top-12 bottom-0 right-0" >
            <div v-for="(report,index) in selectedTechReports" :key="index" class="relative p-1 border-b border-gray-200 cursor-pointer hover:bg-gray-100 duration-300" >
          ...
          <div class="absolute left-0 top-0 right-0 h-10 p-2 font-moul border-b border-gray-200 leading-7 " >
            លទ្ធផលបច្ចេកទេស នៃ កិច្ចប្រជុំ
            ...
            <n-drawer v-model:show="reportUploadVariables.show" placement="right" :width="500" :onAfterLeave="clearReportUpload" >
              <n-drawer-content >
                <template #header >
                  បញ្ចូលលទ្ធផលបច្ចេកទេស នៃ កិច្ចប្រជុំ
                ...
                <input type="file" multiple @change="fileChangeReports" class="hidden" id="tech_reports" />
...
setup(props){
  ...
  function clickUploadReports(){
    document.getElementById('tech_reports').click()
  }
  ...
  function uploadReports(){
    ...
    store.dispatch('meeting/uploadTechReports', formData )
    .then( res => {
      notify.success({
        title: 'រក្សារទុកព័ត៌មាន' ,
      ...
      props.record.tech_reports = res.data.record.tech_reports
      selectedTechReports.value = props.record.tech_reports
      document.getElementById('tech_reports').value = []
  ...
  function previewReports(report){
    store.dispatch('meeting/readTechReport',{
      id: props.record.id ,
  ...
  function removeReport(report){
    dialog.warning({
      ...
      negativeText: 'ទេ',
      onPositiveClick: () => {
        store.dispatch('meeting/removeTechReport',{
          id: props.record.id ,
          serial: report.serial
        })
        .then( res => {
          props.record.tech_reports = res.data.record.tech_reports
          selectedTechReports.value = res.data.record.tech_reports
        }).catch( err => {
          console.log( err )
          notify.error({
            title: 'លុបរបាយណ៍' ,
  ...
  function initial(){
    selectedTechReports.value = props.record.tech_reports != undefined && props.record.tech_reports.length > 0 ? props.record.tech_reports : []
  }
