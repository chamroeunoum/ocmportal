        ...
        <div class="card relative border border-gray-300 rounded mx-auto bg-center bg-cover bg-no-repeat overflow-hidden" style="background-image: url('/src/assets/logo.svg'); background-size: 70% 70%;" >
          ...
          <div v-if="record != undefined " class="flex relative overflow-hidden  max-w-max" >
            ...
            <div class="mx-auto p-4 w-full flex flex-wrap" >
              ...
              <div 
                v-if="record.organization != null && record.organization != undefined && record.organization.length > 0 "
                class="w-full"
                >
                <div class="w-full" >
                  <div class="w-full h-6 text-center mx-auto my-2 font-bold " >{{ record.organization.prefix != "" && record.organization.prefix != undefined && record.organization.prefix.length > 0 ? record.organization.prefix : 'OCM' }}{{ record.organization.prefix != "" && record.organization.prefix != undefined && record.organization.prefix.length > 0 ? "-" + record.organization.prefix : '-' + record.id }}</div>
                </div>
              </div>
              <div class="font-moul text-left mx-auto" >
                <table class="" >
                  ...
                  <tr >
                    ...
                    <td class="font-moul p-1 text-xl leading-6" style="vertical-align: middle ; text-align: left;"  >
                      <div class="mb-2 font-moul" >{{ record.people.lastname + " " + record.people.firstname }}<br/></div>
                      <div class="" >{{ record.people.enlastname + " " + record.people.enfirstname }}<br/></div>
                    </td>
                  </tr>
                  <tr v-if="record != null && record.organization != null && record.organization != undefined " >
                    <td class="w-24 p-1 font-moul leading-6" style="vertical-align: top; text-align: left;"  >អង្គភាព</td>
                    <td class="font-moul p-1 leading-6" style="vertical-align: top; text-align: left;"  >{{ record.organization.name }}</td>
                  </tr>
                  <tr v-if="record != null && record.position != null && record.position != undefined ">
                    <td class="w-24 p-1 font-moul leading-6" style="vertical-align: top; text-align: left;"  >តួនាទី</td>
                    <td class="font-moul p-1 leading-6" style="vertical-align: top; text-align: left;"  >{{ record.position.name }}</td>
                  </tr>
                  </tbody>
                </table>
              </div>
