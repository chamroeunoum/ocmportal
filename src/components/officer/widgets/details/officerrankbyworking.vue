    ...
    <div v-if="show" class="birth-information absolute left-0 right-0 bottom-0 top-0 bg-white" >
        <Transition name="slide-fade" >
            <div v-if="record != undefined && record != null " class="absolute left-0 right-0 bottom-0 top-0 p-8 mb-0" >
                <div class="font-moul border-b border-gray-200 w-full pb-2 mb-4 h-10 leading-6 relative" >ការដំឡើងឋានន្តរស័ក្តិ និងថ្នាក់តាមវេនជ្រើសរើស អតីតភាព ប្ដូរប្រភេទក្របខ័ណ្ឌ និងនិយ័តកម្មថ្នាក់
                ...
                <n-scrollbar >
                    <div class="body" >
                        <table class="w-full" >
                            <thead>
                                <tr class="w-full " >
                                    ...
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black " >ថ្ងៃខែឆ្នាំសុពលភាព</th>
                                    ...
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black " >ការិយាល័យ-ផ្នែក</th>
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black " >ក្របខ័ណ្ឌឋានន្តរស័ក្តិ និងថ្នាក់ចាស់</th>
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black " >ក្របខ័ណ្ឌឋានន្តរស័ក្តិ និងថ្នាក់ថ្មី</th>
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black " >ប្រភេទដំឡើង/ប្ដូរ</th>
                                    <th class="px-1 py-2 bg-gray-200 font-btb-black w-28" >ប្រតិបត្តិការ</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(officerRank , index) in officerRanks" :key="index" class="certificate relative w-full rounded-lg shadow bg-white mb-2 " >
                                    <td class="w-12 index p-1 text-center font-bold" >{{ $toKhmer( index + 1 ) }}</td>
                                    <td class="w-24 p-1 " >{{ $toKhmer( officerRank.date ) }}</td>
                                    <td class="w-24 certificate_leverl p-1 uppercase text-center" >{{ $toKhmer( officerRank.organization ) }}</td>
                                    <td class="w-48 uppercase text-center" >{{ ( officerRank.sub_organization ) }}</td>
                                    <td class="w-48 uppercase text-center" >{{ ( officerRank.sub_sub_organization ) }}</td>
                                    <td class="w-48 uppercase text-center" >{{ ( officerRank.previousRank != undefined && officerRank.previousRank != null ? officerRank.previousRank.desp : '' ) }}</td>
                                    <td class="w-48 uppercase text-center" >{{ ( officerRank.rank != undefined && officerRank.rank != null ? officerRank.rank.desp : '' ) }}</td>
                                    <td class="w-48 uppercase text-center" >{{ ( parseInt( officerRank.changing_type ) >= 0 ? ( changeTypes.find( (ct) => ct.key == officerRank.changing_type ) ).label : 'គ្មានប្រភេទ' ) }}</td>
                                    <td class="flex w-28" >
                                        <svg 
                                        @click="uploadToggler(officerRank)" 
                                        ...
                                        <svg 
                                        v-if="officerRank.pdf==true"
                                        @click="togglePdfModal(officerRank)"
                        ...
                        <div v-if="officerRanks == undefined || officerRanks == null || ( officerRanks.length <= 0 )" class="p-16 text-center" >
                            មិនអ្វីបង្ហាញឡើយ
                        </div>
                    </div>
                </n-scrollbar>
                        ...
                        <n-scrollbar>
                            <div class="form-panel border border-gray-200 rounded-md m-4  bg-white shadow w-2/3 mx-auto " >
                                <div class="w-full p-4 " >
                                    <div class="relative w-full mb-4 border-b border-gray-200 pb-2 font-moul " >ការដំឡើងឋានន្តរស័ក្តិ និងថ្នាក់
                                        <svg 
                                    ...
                                    <n-form class="flex flex-wrap w-full " >
                                        <n-form-item label="ថ្ងៃខែឆ្នាំសុពលភាព" class="w-1/2 p-1" >
                                        ...
                                        <n-form-item label="នាយកដ្ឋាន-អង្គភាព" class="w-1/2 p-1" >
                                            <n-input v-model:value="officerRank.sub_organization" placeholder="នាយកដ្ឋាន-អង្គភាព" />
                                        </n-form-item>
                                        <n-form-item label="ការិយាល័យ-ផ្នែក" class="w-1/2 p-1" >
                                            <n-input v-model:value="officerRank.sub_sub_organization" placeholder="ការិយាល័យ-ផ្នែក" />
                                        </n-form-item>
                                        ...
                                        <n-form-item label="ក្របខ័ណ្ឌឋានន្តរស័ក្តិ និងថ្នាក់ថ្មី" class="w-1/2 p-1" >
                                            <n-select v-model:value="officerRank.rank_id" placeholder="ក្របខ័ណ្ឌឋានន្តរស័ក្តិ និងថ្នាក់ថ្មី" :options="ranks" ></n-select>
                                        </n-form-item>
                                        <n-form-item label="វិស័យ" class="w-full p-1" >
                                            <n-radio-group v-model:value="officerRank.changing_type" name="changing_type">
                                                <n-space>
                                                    <n-radio v-for="(type,index) in changeTypes" :key="index"
                                                        :value="type.key"
                                                        :label="type.label"
                                                    />
                                                </n-space>
        ...
        <Transition name="slide-fade" >
            <div v-if="record == undefined || record == null " >មានបញ្ហាក្នុងការបង្ហាញព័ត៌មានដំឡើងឋានន្តរស័ក្កិ និងថ្នាក។</div>
        </Transition>
        <pdf-preview v-model:model="model" v-model:record="selectedRank" v-bind:show="pdfToggle" :onClose="togglePdfModal"/>
    </div>  
</template>
<script >
        ...
        setup(props){
            ...
            function formToggler() {
                ...
                if( formHelper.value == true ){
                    ...
                    officerRank.id = 0
                    officerRank.officer_id = props.record.id
                    officerRank.organization = '' 
                    officerRank.sub_organization = '' 
                    officerRank.sub_sub_organization = '' 
                    officerRank.date = '' 
                    officerRank.rank_id = null
            ...
            function uploadToggler(officerRank) {
                uploadHelper.value = !uploadHelper.value
                if( officerRank == null || officerRank == undefined ){
                    
                }else{
                    selectedRank.value = officerRank
                }
            }

            const pdfToggle = ref(false)
            function togglePdfModal(cert) {
                selectedRank.value = cert == undefined || cert == null ? null : cert 
            ...
            function save(){
                ...
                selectedRank.value != undefined && selectedRank.value != null && selectedRank.value.id > 0 
                    ...
                    ? {
                        ...
                        organization : officerRank.organization ,
                        sub_organization : officerRank.sub_organization ,
                        sub_sub_organization : officerRank.sub_sub_organization ,
                        rank_id : parseInt( officerRank.rank_id ) > 0 ? parseInt( officerRank.rank_id ) : 0 ,
                        previous_rank_id : parseInt( officerRank.previous_rank_id ) > 0 ? parseInt( officerRank.previous_rank_id ) : 0 ,
                        changing_type : parseInt( officerRank.changing_type ) > 0 ? parseInt( officerRank.changing_type ) : 0
                    }
                    // Create
                    : {
                        officer_id : props.record.id ,
            ...
            function edit(language){
                selectedRank.value = language
                if( selectedRank.value == undefined || selectedRank.value == null ){
                    return false
                }

                officerRank.id = selectedRank.value.id
                officerRank.officer_id = selectedRank.value.officer_id
                officerRank.organization = selectedRank.value.organization 
                officerRank.sub_organization = selectedRank.value.sub_organization
                officerRank.sub_sub_organization = selectedRank.value.sub_sub_organization
                officerRank.changing_type = parseInt( selectedRank.value.changing_type )
                officerRank.rank_id = selectedRank.value.rank_id
                officerRank.previous_rank_id = selectedRank.value.previous_rank_id

                date.value = selectedRank.value.date != undefined && selectedRank.value.date != null && selectedRank.value.date.length > 0 ?( new Date( selectedRank.value.date ) ).getTime() : ( new Date( ) ).getTime()
            ...
            function uploadFiles(){
                ...
                formData.append('id', selectedRank.value.id )
